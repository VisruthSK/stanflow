...
need to fit and analyze models. Specifically, those two commands will
load the core stanflow packages: `bayesplot`, `loo`, `posterior`,
`projpred` and `shinystan`, and setup `brms` to use the `cmdstanr`
backend with as many cores as are available on your machine. Those
package are all “[`library()`](https://rdrr.io/r/base/library.html)d” as
well, so we can proceed with our analysis immediately.
