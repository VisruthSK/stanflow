.stan_pkgs <- c(stanflow_pkgs, "rstantools")

#' Find Stan packages + Stan functions used
#'
#' @param path A project directory OR a single file (.R/.Rmd/.Qmd).
#' @return list(packages=character(), functions=character())
stan_usage <- function(path = ".") {
  files <- if (dir.exists(path)) {
    list.files(
      path,
      recursive = TRUE,
      full.names = TRUE,
      ignore.case = TRUE,
      pattern = "\\.(R|Rmd|Qmd)$"
    )
  } else {
    path
  }

  used_pkgs <- character()
  used_keys <- character()

  for (file in files) {
    code <- .tangle(file)
    exprs <- tryCatch(parse(text = code), error = function(e) NULL)
    if (is.null(exprs)) {
      next
    }
    hits <- .scan(exprs)
    used_pkgs <- c(used_pkgs, hits$pkgs)
    used_keys <- c(used_keys, hits$keys)
  }

  list(packages = sort(unique(used_pkgs)), functions = sort(unique(used_keys)))
}


.fun_to_pkgs <- local({
  funs <- unlist(.stan_exports, use.names = FALSE)
  pkgs <- rep(names(.stan_exports), lengths(.stan_exports))
  split(pkgs, funs)
})

stan_usage <- function(path = ".") {
  files <- list.files(
    path,
    recursive = TRUE,
    full.names = TRUE,
    ignore.case = TRUE,
    pattern = "\\.(R|Rmd|Qmd)$"
  )

  used_pkgs <- character()
  used_keys <- character()

  for (file in files) {
    code <- .tangle(file)
    exprs <- tryCatch(parse(text = code), error = function(e) NULL)
    if (is.null(exprs)) {
      next
    }
    hits <- .scan(exprs)
    used_pkgs <- c(used_pkgs, hits$pkgs)
    used_keys <- c(used_keys, hits$keys)
  }

  list(packages = sort(unique(used_pkgs)), functions = sort(unique(used_keys)))
}

.tangle <- function(file) {
  ext <- tolower(sub(".*\\.", "", file))
  if (ext == "r") {
    return(paste(readLines(file, warn = FALSE), collapse = "\n"))
  }

  tmp <- tempfile(fileext = ".R")
  on.exit(unlink(tmp), add = TRUE)

  if (ext == "rmd") {
    knitr::purl(file, tmp, quiet = TRUE, documentation = 0)
  }
  if (ext == "qmd") {
    quarto::qmd_to_r_script(file, tmp)
  }

  paste(readLines(tmp, warn = FALSE), collapse = "\n")
}

.scan <- function(exprs) {
  pkgs <- character()
  keys <- character()

  walk <- function(x) {
    if (is.expression(x)) {
      return(invisible(lapply(as.list(x), walk)))
    }
    if (!rlang::is_call(x)) {
      return(invisible())
    }

    ns <- rlang::call_ns(x)
    fn <- rlang::call_name(x)

    if (!is.null(ns) && ns %in% .stan_pkgs && !is.null(fn)) {
      pkgs <<- c(pkgs, ns)
      keys <<- c(keys, paste0(ns, "::", fn))
    } else if (
      !is.null(fn) && fn %in% c("library", "require", "requireNamespace")
    ) {
      a <- x[[2]]
      p <- if (is.symbol(a)) {
        as.character(a)
      } else if (is.character(a)) {
        a[[1]]
      } else {
        ""
      }
      if (nzchar(p) && p %in% .stan_pkgs) pkgs <<- c(pkgs, p)
    } else if (!is.null(fn)) {
      hit_pkgs <- .fun_to_pkgs[[fn]]
      if (length(hit_pkgs)) {
        pkgs <<- c(pkgs, hit_pkgs)
        keys <<- c(keys, paste0(hit_pkgs, "::", fn))
      }
    }

    invisible(lapply(as.list(x), walk))
  }

  walk(exprs)
  list(pkgs = pkgs, keys = keys)
}


# generated once--regenerate on releases? just do dynamically?
.stan_exports <-
  list(
    bayesplot = c(
      "ppc_stat_data",
      "example_yrep_draws",
      "mcmc_trace_data",
      "mcmc_scatter",
      "xaxis_ticks",
      "trace_style_np",
      "mcmc_areas_ridges_data",
      "bayesplot_theme_set",
      "ppd_ecdf_overlay",
      "legend_move",
      "scatter_style_np",
      "ppd_freqpoly_grouped",
      "ppc_scatter",
      "yaxis_ticks",
      "mcmc_rhat",
      "example_y_data",
      "ppc_intervals_data",
      "pairs_style_np",
      "example_group_data",
      "ppc_stat",
      "ppc_stat_grouped",
      "ppc_scatter_avg",
      "ppc_ribbon_grouped",
      "lbub",
      "mcmc_hex",
      "ppc_loo_pit_overlay",
      "panel_bg",
      "param_range",
      "legend_none",
      "ppd_stat_freqpoly",
      "mcmc_intervals_data",
      "ppd_dens_overlay",
      "ppc_bars_grouped",
      "ppc_hist",
      "mcmc_recover_scatter",
      "mcmc_trace_highlight",
      "ppd_stat_data",
      "ppc_loo_pit_ecdf",
      "ppc_error_scatter_avg",
      "ppc_scatter_data",
      "ppc_pit_ecdf",
      "mcmc_neff",
      "ppc_loo_ribbon",
      "ppc_loo_intervals",
      "mcmc_rhat_data",
      "theme_default",
      "mcmc_intervals",
      "ppc_stat_freqpoly",
      "ppc_dens_overlay",
      "vline_at",
      "mcmc_trace",
      "facet_bg",
      "mcmc_nuts_stepsize",
      "ppd_ribbon_data",
      "yaxis_text",
      "mcmc_neff_data",
      "mcmc_pairs",
      "ppd_stat",
      "bayesplot_theme_replace",
      "ppc_dots",
      "facet_text",
      "mcmc_parcoord_data",
      "mcmc_areas_ridges",
      "mcmc_recover_hist",
      "mcmc_neff_hist",
      "mcmc_acf",
      "ppc_stat_freqpoly_grouped",
      "ppc_bars_data",
      "mcmc_nuts_acceptance",
      "ppc_intervals",
      "ppd_hist",
      "ppc_ecdf_overlay_grouped",
      "bayesplot_theme_get",
      "mcmc_areas_data",
      "ppd_stat_freqpoly_grouped",
      "ppc_error_scatter_avg_grouped",
      "hline_at",
      "ppc_scatter_avg_data",
      "bayesplot_grid",
      "ppc_freqpoly",
      "color_scheme_view",
      "mcmc_dens_chains_data",
      "nuts_params",
      "ppd_ribbon_grouped",
      "ppc_ribbon",
      "ppc_stat_2d",
      "ppc_error_hist",
      "legend_text",
      "overlay_function",
      "ppc_loo_pit",
      "available_mcmc",
      "ppc_dens",
      "mcmc_areas",
      "mcmc_violin",
      "ppc_dens_overlay_grouped",
      "ppd_dots",
      "ppc_loo_pit_data",
      "ppc_rootogram",
      "ppd_intervals_data",
      "bayesplot_theme_update",
      "mcmc_recover_intervals",
      "ppc_ribbon_data",
      "ppc_boxplot",
      "ppd_intervals",
      "mcmc_combo",
      "mcmc_nuts_energy",
      "ppc_data",
      "ppc_bars",
      "example_mcmc_draws",
      "mcmc_nuts_treedepth",
      "mcmc_hist_by_chain",
      "vars",
      "ppd_dens",
      "ppc_violin_grouped",
      "neff_ratio",
      "abline_01",
      "ppd_stat_2d",
      "mcmc_dens_chains",
      "xaxis_text",
      "ppd_freqpoly",
      "ppc_freqpoly_grouped",
      "xaxis_title",
      "ppc_loo_pit_qq",
      "param_glue",
      "rhat",
      "ppd_stat_grouped",
      "color_scheme_get",
      "parcoord_style_np",
      "mcmc_dens",
      "ppc_ecdf_overlay",
      "vline_0",
      "mcmc_rank_hist",
      "plot_bg",
      "ppd_data",
      "mcmc_dens_overlay",
      "ppd_ribbon",
      "mcmc_parcoord",
      "ppc_error_binned",
      "ppc_km_overlay_grouped",
      "yaxis_title",
      "available_ppc",
      "ppd_intervals_grouped",
      "mcmc_hist",
      "mcmc_rhat_hist",
      "color_scheme_set",
      "available_ppd",
      "ppc_scatter_avg_grouped",
      "mcmc_nuts_divergence",
      "ppd_boxplot",
      "ppc_error_scatter",
      "pp_check",
      "ppc_error_data",
      "ppc_pit_ecdf_grouped",
      "hline_0",
      "mcmc_rank_ecdf",
      "log_posterior",
      "mcmc_rank_overlay",
      "example_x_data",
      "ppc_error_hist_grouped",
      "grid_lines",
      "pairs_condition",
      "mcmc_acf_bar",
      "ppc_intervals_grouped",
      "ppc_km_overlay",
      "ppc_error_scatter_avg_vs_x"
    ),
    loo = c(
      "loo_approximate_posterior.array",
      "loo_i",
      "pareto_k_ids",
      ".thin_draws",
      "crps",
      "loo_approximate_posterior.function",
      "waic.array",
      "loo_predictive_metric",
      "pareto_k_influence_values",
      ".ndraws",
      "loo_subsample.function",
      "obs_idx",
      "pareto_k_table",
      "is.psis_loo",
      "is.psis",
      "scrps",
      "waic",
      "relative_eff",
      "loo_compare",
      "loo_model_weights.default",
      "pseudobma_weights",
      "nlist",
      "loo_subsample",
      "kfold_split_grouped",
      "kfold_split_stratified",
      "is.waic",
      "kfold_split_random",
      "is.sis",
      "is.loo",
      "stacking_weights",
      "psis_n_eff_values",
      ".compute_point_estimate",
      "is.tis",
      "elpd",
      "loo_approximate_posterior",
      "waic.function",
      "gpdfit",
      "psis",
      "psislw",
      "loo_approximate_posterior.matrix",
      "pointwise",
      "extract_log_lik",
      "is.kfold",
      "loo_moment_match",
      "example_loglik_array",
      "mcse_loo",
      "loo_model_weights",
      "loo_moment_match.default",
      "compare",
      "loo_crps",
      "sis",
      "loo.matrix",
      "find_model_names",
      "loo",
      "E_loo",
      "loo.function",
      "tis",
      "kfold",
      "loo_scrps",
      "example_loglik_matrix",
      "pareto_k_values",
      "weights.importance_sampling",
      "print_dims",
      "loo.array",
      "waic.matrix"
    ),
    posterior = c(
      "rvar_is_na",
      "drop",
      "as_draws_df",
      "niterations",
      "pareto_min_ss",
      "rvar_any",
      "rvar_min",
      "mcse_median",
      "extract_variable_array",
      "mcse_sd",
      "is_rvar",
      "rvar_mad",
      "var",
      "is_draws_list",
      "autocovariance",
      "ess_bulk",
      "diag",
      "as_rvar_ordered",
      "ess_basic",
      "rvar_mean",
      "pareto_convergence_rate",
      "rvar_prod",
      "autocorrelation",
      "for_each_draw",
      "ess_quantile",
      "rvar_sum",
      "is_draws_matrix",
      "rename_variables",
      "as_draws_array",
      "default_convergence_measures",
      "bind_draws",
      "mad",
      "draws_rvars",
      "as_draws_list",
      "mcse_mean",
      "rvar_max",
      "order_draws",
      "match",
      "as_rvar_factor",
      "ps_tail_length",
      "pareto_smooth",
      "iteration_ids",
      "subset_draws",
      "rdo",
      "rvar_ifelse",
      "ps_min_ss",
      "nchains",
      "is_draws",
      "rvar_var",
      "draws_list",
      "variables",
      "dissent",
      "rstar",
      "summarise_draws",
      "weight_draws",
      "summarize_draws",
      "mcse_quantile",
      "ndraws",
      "is_draws_array",
      "draw_ids",
      "rvar_range",
      "mutate_variables",
      "pareto_khat_threshold",
      "rhat_basic",
      "ess_mean",
      "is_rvar_factor",
      "thin_draws",
      "sd",
      "rhat_nested",
      "split_chains",
      "draws_of<-",
      "pareto_diags",
      "rvar_is_infinite",
      "as_draws_rvars",
      "ess_sd",
      "default_mcse_measures",
      "rvar_factor",
      "ps_khat_threshold",
      "z_scale",
      "r_scale",
      "as_rvar_integer",
      "Pr",
      "%**%",
      "rhat",
      "as_draws_matrix",
      "E",
      "draws_matrix",
      "set_variables",
      "quantile2",
      "cdf",
      "chain_ids",
      "variables<-",
      "rvar_rng",
      "nvariables",
      "example_draws",
      "is_draws_rvars",
      "%in%",
      "rvar_is_finite",
      "as_rvar",
      "modal_category",
      "is_rvar_ordered",
      "rvar_ordered",
      "rvar_median",
      "pareto_khat",
      "rvar_apply",
      "as_rvar_logical",
      "rvar_is_nan",
      "extract_variable_matrix",
      "rvar_sd",
      "draws_array",
      "merge_chains",
      "draws_df",
      "rfun",
      "u_scale",
      "repair_draws",
      "as_rvar_numeric",
      "rvar",
      "variance",
      "ess_median",
      "as_draws",
      "rvar_quantile",
      "resample_draws",
      "ess_tail",
      "default_summary_measures",
      "rvar_all",
      "ps_convergence_rate",
      "entropy",
      "reserved_variables",
      "draws_of",
      "is_draws_df",
      "extract_variable"
    ),
    projpred = c(
      "proj_linpred",
      "varsel",
      "extend_family",
      "do_call",
      "get_refmodel",
      "Student_t",
      "augdat_link_binom",
      "proj_predict",
      "project",
      "predictor_terms",
      "run_cvfun",
      "cv_varsel",
      "solution_terms",
      "force_search_terms",
      "ranking",
      "init_refmodel",
      "break_up_matrix_term",
      "cl_agg",
      "suggest_size",
      "y_wobs_offs",
      "performances",
      "cv_ids",
      "cv_proportions",
      "cv_folds",
      "cvfolds",
      "augdat_ilink_binom"
    ),
    shinystan = c(
      "retrieve",
      "update_sso",
      "model_name",
      "model_code",
      "rename_model",
      "notes",
      "drop_parameters",
      "sso_info",
      "generate_quantity",
      "is.shinystan",
      "as.shinystan",
      "launch_shinystan",
      "launch_shinystan_demo",
      "deploy_shinystan"
    ),
    brms = c(
      "loo_moment_match",
      "phurdle_gamma",
      "zero_inflated_beta",
      "cor_errorsar",
      "is.cor_sar",
      "unstr",
      "prepare_predictions",
      "posterior_smooths",
      "do_call",
      "resp_vreal",
      "is.brmsfit",
      "rbeta_binomial",
      "qgen_extreme_value",
      "horseshoe",
      "pshifted_lnorm",
      "zero_inflated_poisson",
      "add_criterion",
      "cor_sar",
      "wiener",
      "marginal_smooths",
      "resp_cens",
      "as_draws",
      "weibull",
      "dzero_inflated_beta_binomial",
      "ngrps",
      "cor_bsts",
      "cs",
      "dstudent_t",
      "qskew_normal",
      "posterior_interval",
      "loo_compare",
      "gen_extreme_value",
      "phurdle_poisson",
      "sratio",
      "resp_mi",
      "bridge_sampler",
      "sar",
      "data_predictor",
      "is.cor_brms",
      "brmsterms",
      "loo_linpred",
      "posterior_epred",
      "posterior_table",
      "asym_laplace",
      "dhurdle_poisson",
      "nlf",
      "geometric",
      "conditional_smooths",
      "theme_black",
      "marginal_effects",
      "as.mcmc",
      "pbeta_binomial",
      "hurdle_cumulative",
      "mixture",
      "rgen_extreme_value",
      "loo_subsample",
      "frechet",
      "predictive_interval",
      "phurdle_negbinomial",
      "dzero_inflated_poisson",
      "expp1",
      "is.cor_arma",
      "get_dpar",
      "validate_prior",
      "rvon_mises",
      "zero_inflated_beta_binomial",
      "ranef",
      "predictive_error",
      "loo_epred",
      "rmulti_normal",
      "inv_logit_scaled",
      "validate_newdata",
      "rwiener",
      "resp_bhaz",
      "resp_dec",
      "mmc",
      "variables",
      "dlogistic_normal",
      "kfold",
      "rfrechet",
      "mcmc_plot",
      "recompile_model",
      "cumulative",
      "prior_",
      "lasso",
      "resp_index",
      "bernoulli",
      "prior_samples",
      "pp_expect",
      "brmsfit_needs_refit",
      "LOO",
      "cox",
      "gp",
      "get_y",
      "gr",
      "get_prior",
      "log_lik",
      "resp_subset",
      "is.mvbrmsformula",
      "cor_ar",
      "R2D2",
      "is.brmsterms",
      "add_waic",
      "hurdle_negbinomial",
      "kfold_predict",
      "dgen_extreme_value",
      "brm_multiple",
      "dshifted_lnorm",
      "parse_bf",
      "pinv_gaussian",
      "make_conditions",
      "student",
      "set_nl",
      "rshifted_lnorm",
      "mvbf",
      "exgaussian",
      "as_draws_array",
      "fcor",
      "is.cor_car",
      "conditional_effects",
      "beta_binomial",
      "exponential",
      "qstudent_t",
      "data_response",
      "make_standata",
      "mvbrmsformula",
      "cor_car",
      "resp_thres",
      "s",
      "qshifted_lnorm",
      "dzero_inflated_beta",
      "dwiener",
      "dfrechet",
      "dskew_normal",
      "acat",
      "qasym_laplace",
      "cse",
      "pp_average",
      "pexgaussian",
      "cratio",
      "car",
      "pp_check",
      "add_ic",
      "zero_inflated_negbinomial",
      "negbinomial",
      "pstudent_t",
      "stanplot",
      "theme_default",
      "pp_mixture",
      "loo",
      "reloo",
      "dvon_mises",
      "posterior_samples",
      "as_draws_list",
      "brm",
      "dasym_laplace",
      "as_draws_df",
      "prior_string",
      "dhurdle_gamma",
      "logm1",
      "hypothesis",
      "WAIC",
      "cor_fixed",
      "dmulti_normal",
      "hurdle_lognormal",
      "lf",
      "rinv_gaussian",
      "zero_one_inflated_beta",
      "cor_arr",
      "save_pars",
      "rlogistic_normal",
      "neff_ratio",
      "ma",
      "post_prob",
      "loo_R2",
      "set_prior",
      "empty_prior",
      "resp_weights",
      "expose_functions",
      "nuts_params",
      "nvariables",
      "update_adterms",
      "shifted_lognormal",
      "read_csv_as_stanfit",
      "pfrechet",
      "pzero_inflated_beta_binomial",
      "me",
      "rasym_laplace",
      "density_ratio",
      "mi",
      "rows2labels",
      "dmulti_student_t",
      "mm",
      "make_stancode",
      "pzero_inflated_poisson",
      "mo",
      "cor_arma",
      "cor_icar",
      "logit_scaled",
      "mvbind",
      "dhurdle_negbinomial",
      "qfrechet",
      "resp_se",
      "posterior_summary",
      "bayes_R2",
      "VarCorr",
      "combine_models",
      "control_params",
      "pzero_inflated_binomial",
      "ddirichlet",
      "hurdle_poisson",
      "fixef",
      "cor_cosy",
      "resp_trunc",
      "nsamples",
      "nchains",
      "loo_predictive_interval",
      "rhat",
      "add_ic<-",
      "cosy",
      "hurdle_gamma",
      "brmsformula",
      "resp_trials",
      "skew_normal",
      "pzero_inflated_negbinomial",
      "categorical",
      "dinv_gaussian",
      "parnames",
      "extract_draws",
      "zero_inflated_binomial",
      "is.cor_fixed",
      "rstudent_t",
      "prior_summary",
      "arma",
      "custom_family",
      "threading",
      "Beta",
      "prior",
      "bayes_factor",
      "t2",
      "niterations",
      "rename_pars",
      "dhurdle_lognormal",
      "rmulti_student_t",
      "dzero_inflated_binomial",
      "pasym_laplace",
      "restructure",
      "pzero_inflated_beta",
      "standata",
      "model_weights",
      "ndraws",
      "waic",
      "pskew_normal",
      "posterior_linpred",
      "logistic_normal",
      "dirichlet",
      "dexgaussian",
      "add_loo",
      "brmsfamily",
      "stanvar",
      "default_prior",
      "log_posterior",
      "von_mises",
      "resp_vint",
      "rdirichlet",
      "rexgaussian",
      "is.brmsformula",
      "is.cor_cosy",
      "is.brmsprior",
      "set_rescor",
      "cor_lagsar",
      "compare_ic",
      "loo_model_weights",
      "constant",
      "multinomial",
      "resp_rate",
      "autocor",
      "is.mvbrmsterms",
      "pvon_mises",
      "lognormal",
      "pgen_extreme_value",
      "posterior_average",
      "as_draws_matrix",
      "cor_ma",
      "acformula",
      "ar",
      "as.brmsprior",
      "psis",
      "prior_draws",
      "add_rstan_model",
      "phurdle_lognormal",
      "loo_predict",
      "dzero_inflated_negbinomial",
      "stancode",
      "is.brmsfit_multiple",
      "bf",
      "set_mecor",
      "as_draws_rvars",
      "dbeta_binomial",
      "resp_cat",
      "posterior_predict",
      "rskew_normal",
      "opencl"
    ),
    cmdstanr = c(
      "cmdstan_version",
      "as.CmdStanGQ",
      "as.CmdStanDiagnose",
      "as_mcmc.list",
      "cmdstan_default_install_path",
      "rebuild_cmdstan",
      "num_threads",
      "write_stan_tempfile",
      "as_draws",
      "as.CmdStanLaplace",
      "cmdstanr_example",
      "as.CmdStanPathfinder",
      "read_sample_csv",
      "as_cmdstan_fit",
      "cmdstan_make_local",
      "set_cmdstan_path",
      "cmdstan_model",
      "write_stan_file",
      "check_cmdstan_toolchain",
      "cmdstan_default_path",
      "as.CmdStanMCMC",
      "cmdstan_path",
      "as.CmdStanVB",
      "register_knitr_engine",
      "draws_to_csv",
      "write_stan_json",
      "install_cmdstan",
      "as.CmdStanMLE",
      "read_cmdstan_csv",
      "print_example_program",
      "set_num_threads",
      "eng_cmdstan"
    ),
    rstan = c(
      "get_seeds",
      "get_posterior_mean",
      "plot",
      "get_num_leapfrog_per_iteration",
      "check_divergences",
      "stan_plot",
      "rstan.package.skeleton",
      "stan_demo",
      "loo",
      "stan_trace",
      "stan",
      "get_bfmi",
      "stan_version",
      "ess_bulk",
      "traceplot",
      "check_hmc_diagnostics",
      "extract",
      "optimizing",
      "log_prob",
      "get_max_treedepth_iterations",
      "sbc",
      "sflist2stanfit",
      "quietgg",
      "rstan_options",
      "stan_model",
      "get_cppcode",
      "rstan_gg_options",
      "check_treedepth",
      "read_rdump",
      "summary",
      "get_rng",
      "get_inits",
      "stan_hist",
      "constrain_pars",
      "get_num_divergent",
      "check_energy",
      "stan_diag",
      "stan_ess",
      "show",
      "get_cxxflags",
      "stanc_builder",
      "stan_scat",
      "get_adaptation_info",
      "stan_rdump",
      "unconstrain_pars",
      "rstan_ggtheme_options",
      "stanc",
      "stan_par",
      "get_stanmodel",
      "sampling",
      "get_num_upars",
      "nlist",
      "extract_sparse_parts",
      "stan_ac",
      "read_stan_csv",
      "cpp_object_initializer",
      "monitor",
      "makeconf_path",
      "lookup",
      "Rhat",
      "set_cppo",
      "get_low_bfmi_chains",
      "stan_rhat",
      "stan_mcse",
      "stan_dens",
      "get_divergent_iterations",
      "As.mcmc.list",
      "expose_stan_functions",
      "get_seed",
      "get_stream",
      "gqs",
      "get_logposterior",
      "grad_log_prob",
      "vb",
      "get_stancode",
      "loo_moment_match",
      "get_cppo_mode",
      "ess_tail",
      "get_sampler_params",
      "get_elapsed_time",
      "get_num_max_treedepth"
    ),
    rstanarm = c(
      "posterior_epred",
      "stan_jm",
      "ranef",
      "stan_mvmer",
      "loo",
      "stan_gamm4",
      "laplace",
      "loo_compare",
      "stanreg_list",
      "stan_clogit",
      "se",
      "stan_glm",
      "stan_betareg.fit",
      "stan_lm.fit",
      "get_x",
      "predictive_interval",
      "stanmvreg_list",
      "get_y",
      "get_z",
      "VarCorr",
      "kfold",
      "posterior_predict",
      "default_prior_intercept",
      "as_draws",
      "loo_linpred",
      "pp_validate",
      "lkj",
      "stan_glmer",
      "stan_glm.fit",
      "default_prior_coef",
      "waic",
      "hs",
      "as_draws_matrix",
      "lasso",
      "exponential",
      "loo_predictive_interval",
      "sigma",
      "plot_nonlinear",
      "stan_nlmer",
      "posterior_linpred",
      "nsamples",
      "pp_check",
      "bayes_R2",
      "stan_glm.nb",
      "decov",
      "stan_lm.wfit",
      "pairs_style_np",
      "stan_glmer.nb",
      "ps_check",
      "hs_plus",
      "normal",
      "stanjm_list",
      "logit",
      "cauchy",
      "as_draws_df",
      "launch_shinystan",
      "stan_lm",
      "plot_stack_jm",
      "predictive_error",
      "log_lik",
      "prior_options",
      "posterior_survfit",
      "loo_R2",
      "stan_biglm.fit",
      "prior_summary",
      "posterior_traj",
      "posterior_vs_prior",
      "student_t",
      "product_normal",
      "stan_polr.fit",
      "loo_predict",
      "R2",
      "dirichlet",
      "fixef",
      "pairs_condition",
      "stan_polr",
      "compare_models",
      "invlogit",
      "as_draws_array",
      "stan_aov",
      "loo_model_weights",
      "as_draws_rvars",
      "stan_lmer",
      "ngrps",
      "stan_betareg",
      "posterior_interval",
      "as_draws_list",
      "stan_biglm",
      "Surv",
      "neg_binomial_2"
    ),
    rstantools = c(
      "loo_linpred",
      "log_lik",
      "predictive_interval",
      "loo_epred",
      "loo_predictive_interval",
      "loo_pit",
      "use_rstan",
      "posterior_linpred",
      "posterior_epred",
      "posterior_predict",
      "loo_predict",
      "rstantools_load_code",
      "predictive_error",
      "prior_summary",
      "loo_R2",
      "bayes_R2",
      "posterior_interval",
      "nsamples",
      "init_cpp",
      "rstan_create_package",
      "rstan_config"
    )
  )
