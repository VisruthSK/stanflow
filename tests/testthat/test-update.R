...
test_that("stanflow_update refreshes citation cache for updated packages", {
  withr::local_options(
    list(
      repos = c(CRAN = "https://cloud.r-project.org"),
      stanflow.testing = FALSE,
  ...
  reset_cache <- getFromNamespace(".reset_citation_cache", "stanflow")
  withr::defer(reset_cache(c("posterior", "loo")))

  versions <- list(updated = "1.0.0", untouched = "1.0.0")
  original_package_description <- utils::packageDescription
  local_mocked_bindings(
    packageDescription = function(pkg, ...) {
      if (!pkg %in% c("posterior", "loo")) {
        return(original_package_description(pkg, ...))
      }
      version <- if (identical(pkg, "posterior")) {
        versions$updated
      } else {
        versions$untouched
      }
  ...
  reset_cache(c("posterior", "loo"))

  tmp <- withr::local_tempdir()
  path <- file.path(tmp, "script.R")
  writeLines(c("posterior::as_draws(1)", "loo::compare(1, 1)"), path)

  before <- paste(stan_cite(path), collapse = "\n")
  expect_true(grepl("R package version 1.0.0", before, fixed = TRUE))
  expect_false(grepl("R package version 2.0.0", before, fixed = TRUE))
  expect_false(grepl("R package version 9.9.9", before, fixed = TRUE))
  ...
  with_mocked_bindings(
    ...
    with_mocked_bindings(
      install.packages = function(...) {
        ...
        versions$untouched <<- "9.9.9"
        invisible(NULL)
      },
      capture.output(stanflow_update()),
      .package = "utils"
    ),
    .package = "stanflow"
  )

  after <- paste(stan_cite(path), collapse = "\n")
  expect_true(grepl("R package version 2.0.0", after, fixed = TRUE))
  expect_true(grepl("R package version 1.0.0", after, fixed = TRUE))
  expect_false(grepl("R package version 9.9.9", after, fixed = TRUE))
})
